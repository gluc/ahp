...
When deciding which car to buy, the family considers the following criteria: cost, safety, style, capacity. Additionally, some of the criteria have sub-criteria. 
...
What is more important, style or safety? AHP allows expressing an individual's or a group's opinion by polling the pairwise preferences on a scale of 1 to 9, and their inverse. 1 means two categories are equally important. 9 means A is strictly preferred to B. 1/9 means B is strictly preferable to A.

In order to apply the AHP algorithm by calculating the *priorities*, we need to construct a *preference matrix*. The ahp package provides a utility method to construct such a matrix. The basic workflow is this, for each criteria node:

* define your preference matrix for the node's children. There are two ways to do this:
    + directly, all by yourself
    + use the helper methods `printForInput` and `AhpMatrix`
* add the preference matrix to the node by calling `node$SetChildPreferenceMatrix(preferenceMatrix)`
* review the priorities and the consistency by calling
    + `priorities(node)`
    + node$consistency
...
We start by setting the level one criteria preferences on the root, our `goal` node. These level one criterias are:
...
The idea of `printForInput` is that we copy and paste what is printed to the console when calling `printForInput`, and then replace the 1's with the appropriate subjective values:
...
A technical side note: you may have realized that `pp`, the pairwise child combinations of our categories, is a standard `data.frame`, and not an R6 reference class. As a result, we must re-assign `pp`, using e.g. `pp <- setPreference(pp, "Cost"   , "Safety"  , 3)`.
...
```{r}
print(goal, "priority", "globalPriority", "consistency")
```

## Sub-Criteria

The AHP tree still looks incomplete. We need to set pairwise preferences for each combination, at each node. Let us continue with the preferences of the sub-categories below the *Cost* node.

### Cost

Just as above, we use the `printForInput` helper:

```{r}
pp <- goal$Cost$GetChildCombinations()
#printForInput(pp)
...
preferenceMatrix <- AhpMatrix(pp)
goal$Cost$SetChildPreferenceMatrix(preferenceMatrix)

priorities(goal$Cost)
```

Note that the sum of the priorities is equal to 1:

```{r}
sum(priorities(goal$Cost))
```
...
```{r}
pp <- goal$Safety$GetChildCombinations()
#printForInput(pp)
...
preferenceMatrix <- AhpMatrix(pp)
goal$Safety$SetChildPreferenceMatrix(preferenceMatrix)

print(goal$Safety, "priority", "globalPriority", "consistency")
...
```{r}
pp <- goal$Capacity$GetChildCombinations()
#printForInput(pp)

pp <- setPreference(pp, "Cargo Capacity", "Passenger Capacity", 1/5)
preferenceMatrix <- AhpMatrix(pp)
goal$Capacity$SetChildPreferenceMatrix(preferenceMatrix)
```
...
We establish that consistency is acceptable everywhere, and that for example purchase price is more than 6 times more important than cargo capacity.

## Alternatives

We are now ready to move to the next step: Alternatives.
The Jones family has researched and has 6 cars on their shortlist. Let's create them, analougous to the Nodes:
...
```{r}
print(goal, "priority", "globalPriority", "consistency", limit = 20)
```

The next step is to set our alternative-preferences for each category. The principle is the same as above: for each leaf criteria, we create a preference matrix containing the comparison of all alternative combinations.
...
```{r}
goal$Cost$`Purchase Price`$CalculatePreferences(purchasePricePreference)
...
```{r}
goal$Cost$`Purchase Price`$preferenceMatrix

```

... as well as the priorities:
```{r}
priorities(goal$Cost$`Purchase Price`)
...
```{r}
goal$Cost$`Fuel Costs`$CalculatePreferences(fuelCostFunction)
sort(priorities(goal$Cost$`Fuel Costs`), decreasing = TRUE)
...
```{r}
pp <- goal$Cost$`Maintenance Costs`$GetChildCombinations()
#printForInput(pp)
...
preferenceMatrix <- AhpMatrix(pp)
goal$Cost$`Maintenance Costs`$SetChildPreferenceMatrix(preferenceMatrix)

priorities(goal$Cost$`Maintenance Costs`)
...
goal$Cost$`Resale Value`$CalculatePreferences(resaleValuePreference)
priorities(goal$Cost$`Resale Value`)
...
We use the ahp package utility function `LinearComparison`, assuming that the overall lightest existing car is 3000, and the heaviest is 5000.
Note that we pass along additional parameters to the LinearComparison function.

```{r}
goal$Safety$`Curb Weight`$CalculatePreferences(LinearComparison, 
...
goal$Safety$`Safety Class`$ApplyPreferenceMatrix(preferenceMatrix, 'safetyClass')
print(goal$Safety$`Safety Class`, "priority", "globalPriority", "consistency")
```
...
goal$Safety$`Crash Rating`$ApplyPreferenceMatrix(preferenceMatrix, 'crashRating')
priorities(goal$Safety$`Crash Rating`)
```
...
```{r}
pp <- goal$Style$GetChildCombinations()
#printForInput(pp)
...
preferenceMatrix <- AhpMatrix(pp)
goal$Style$SetChildPreferenceMatrix(preferenceMatrix)

priorities(goal$Style)
...
goal$Capacity$`Cargo Capacity`$CalculatePreferences(cargoPreference)
priorities(goal$Capacity$`Cargo Capacity`)
...
goal$Capacity$`Passenger Capacity`$ApplyPreferenceMatrix(AhpMatrix(pp), 'passenger')

priorities(goal$Capacity$`Passenger Capacity`)
...
Looking at the solution, we might find that the Curb Weight has too much influence, so let's adjust.

First, what was the preference matrix?

```{r}
agoal$Find("Safety")$preferenceMatrix
```
...
pp <- setPreference(pp, "Curb Weight"   , "Safety Class", 2)
pp <- setPreference(pp, "Curb Weight"   , "Crash Rating", 1)
pp <- setPreference(pp, "Safety Class"  , "Crash Rating", 1/2)
