...
```{r}
library(data.tree)
vignette('Introduction', package = 'data.tree')
```
...
The idea of `printForInput` is that we copy and paste the output, and then replace the 1's with the appropriate subjective values:
...
```{r}
goal$Find("Cost", "Purchase Price")$CalculatePreferences(purchasePricePreference)
...
```{r}
goal$Find("Cost", "Purchase Price")$preferenceMatrix

```

... as well as the priorities:
```{r}
priorities(goal$Find("Cost", "Purchase Price"))
...
```{r}
goal$Find("Cost", "Fuel Costs")$CalculatePreferences(fuelCostFunction)
sort(priorities(goal$Find("Cost", "Fuel Costs")), decreasing = TRUE)
...
```{r}
pp <- goal$Find("Cost", "Maintenance Costs")$GetChildCombinations()
#printForInput(pp)
...
preferenceMatrix <- AhpMatrix(pp)
goal$Find("Cost", "Maintenance Costs")$SetChildPreferenceMatrix(preferenceMatrix)

priorities(goal$Find("Cost", "Maintenance Costs"))
...
goal$Find("Cost", "Resale Value")$CalculatePreferences(resaleValuePreference)
priorities(goal$Find("Cost", "Resale Value"))
...
```{r}
goal$Find("Safety", "Curb Weight")$CalculatePreferences(LinearComparison, fieldName = "curb_weight", min_x = 3000, max_x = 5000)
```
...
```{r}
goal$Find("Safety", "Curb Weight")$CalculatePreferences(LinearComparison, fieldName = "curb_weight", min_x = 2000, max_x = 5500)
goal$Find("Safety", "Curb Weight")
```
...
goal$Find("Safety", "Safety Class")$ApplyPreferenceMatrix(preferenceMatrix, 'safetyClass')
priorities(goal$Find("Safety", "Safety Class"))
```
...
goal$Find("Safety", "Crash Rating")$ApplyPreferenceMatrix(preferenceMatrix, 'crashRating')
priorities(goal$Find("Safety", "Crash Rating"))
```
...
goal$Find("Capacity", "Cargo Capacity")$CalculatePreferences(cargoPreference)
priorities(goal$Find("Capacity", "Cargo Capacity"))
...
goal$Find("Capacity", "Passenger Capacity")$ApplyPreferenceMatrix(AhpMatrix(pp), 'passenger')

priorities(goal$Find("Capacity", "Passenger Capacity"))
...
```{r}
goal$Find("Safety")$preferenceMatrix
```
